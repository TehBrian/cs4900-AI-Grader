...
class GradingCoordinator:
    ...
    def grade(self, problem, student_answer):
        ...
        if method == 'symbolic':
            return self.symbolic_grader.grade(student_answer, problem.solution_expression)
        elif method == 'numerical':
            return self.numerical_grader.grade(student_answer, problem.solution_expression, 
                                             tolerance=problem.answer_tolerance or 0.01)
        elif method == 'pattern':
            if ',' in problem.solution_expression:
                return self.pattern_grader.grade_sequence(student_answer, problem.solution_expression)
            else:
                return self.pattern_grader.grade(student_answer, problem.solution_expression)
        else:
            return self.pattern_grader.grade(student_answer, problem.solution_expression)
    ...
    def _detect_grading_method(self, problem, student_answer):
        ...
        if any(op in solution.lower() for op in ['^', '**', 'sin', 'cos', 'tan', 'log', 'sqrt']):
            return 'symbolic'
