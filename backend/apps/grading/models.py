...
class StudentSubmission(models.Model):
    ...
    expected_answer = models.TextField(help_text="Calculated expected answer for these parameters")
    ...
    grading_engine = models.ForeignKey(GradingEngine, on_delete=models.SET_NULL, null=True, blank=True)
    ...
    time_spent = models.FloatField(null=True, blank=True, help_text="Time spent on problem in seconds")
    ...
    def complete_grading(self, is_correct, score, grading_engine=None):
        ...
        self.save(update_fields=['is_correct', 'score', 'status', 'grading_completed_at', 'grading_engine'])
...
class SubmissionPart(models.Model):
    ...
    """
    submission = models.ForeignKey(StudentSubmission, on_delete=models.CASCADE, related_name='parts')
...
class GradingResult(models.Model):
    ...
    """
    submission = models.OneToOneField(StudentSubmission, on_delete=models.CASCADE, related_name='result')
    
    # Detailed grading information
    cas_result = models.JSONField(null=True, blank=True, help_text="Results from CAS evaluation")
    ai_result = models.JSONField(null=True, blank=True, help_text="Results from AI evaluation")
