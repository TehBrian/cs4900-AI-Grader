...
class MultiStepGrader:
    ...
    def grade_submission(
        ...
        if self.has_openai and parsed_submission['work_shown']:
            ...
            work_percentage = (work_score / max_work_score * 100) if max_work_score > 0 else 0
            
            result['total_score'] = (
                final_answer_result['score'] * 0.7 +
                work_percentage * 0.3
            )
            result['confidence'] = (
    ...
    def _grade_work_shown(
        ...
        try:
            ...
            return {
                'step_scores': [],
                'feedback': [{'section': 'Work Evaluation', 'feedback': 'Unable to evaluate work shown'}],
                'confidence': 0.0
            }
    ...
    def _process_gpt_response(self, gpt_result: Dict) -> Dict[str, Any]:
        ...
        if gpt_result.get('strengths'):
            feedback.append({
                'section': 'Strengths',
                'feedback': ', '.join(gpt_result['strengths'])
            })
...
def format_grading_result(result: Dict[str, Any]) -> str:
    ...
    if result.get('step_scores'):
        ...
        for step in result['step_scores']:
            ...
            output.append(f"  {status} Step {step['step_number']}: {step['score']:.0f}/100")
